...
def _handle_PacketIn (event):
  ...
  if(packet.src=="00:00:00:00:00:01"):
    ...
    log.debug("Broadcasting %s", (packet.src))
  ...
  if dst_port is None:
    ...
    log.debug("Packet parse %s", (packet.parse))
    log.debug(dir(packet.parse))
    log.debug("Packet parsed %s", (packet.parsed))
    log.debug(dir(packet.parsed))
    attrs = vars(packet)
    ...
    log.debug("Data out %s", (event.ofp))
    log.debug(dir(event.ofp))
    attrs = vars(event.ofp)
    log.debug(', '.join("%s: %s" % item for item in attrs.items()))
